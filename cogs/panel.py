...
class StatsView(discord.ui.View):
    ...
    async def server_top(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not stats_cog:
            await interaction.response.send_message("❌ Stats не загружен", ephemeral=True)
            return
        
        # Используем метод из Stats
        embed = await stats_cog.get_leaderboard_embed(interaction.guild, 7)
